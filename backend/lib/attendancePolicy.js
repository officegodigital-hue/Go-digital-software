...
let settings = { ...defaults, requiredWorkMinutes: REQUIRED_MINUTES };
...
function isEarlyExit(workingMinutes) { return Number(workingMinutes || 0) < Number(settings.requiredWorkMinutes || REQUIRED_MINUTES); }
...
async function getTimeSettings(db) { await ensureTimeSettings(db); const [rows] = await db.query('SELECT shift_start, shift_end, late_after, absent_after FROM hrms_attendance_time_settings WHERE id = 1'); if (rows[0]) { settings = { shiftStart: String(rows[0].shift_start), shiftEnd: String(rows[0].shift_end), lateAfter: String(rows[0].late_after), absentAfter: String(rows[0].absent_after || defaults.absentAfter), requiredWorkMinutes: REQUIRED_MINUTES }; try { const [policies] = await db.query("SELECT required_work_minutes FROM hrms_attendance_policies WHERE scope_type='global' AND scope_id IS NULL AND is_active=1 AND effective_from <= CURDATE() ORDER BY effective_from DESC,id DESC LIMIT 1"); if (policies[0]) settings.requiredWorkMinutes = Number(policies[0].required_work_minutes || REQUIRED_MINUTES); } catch (_) {} if (timeToSeconds(settings.lateAfter) < timeToSeconds(settings.shiftStart)) settings.lateAfter = settings.shiftStart; if (timeToSeconds(settings.absentAfter) < timeToSeconds(settings.lateAfter)) settings.absentAfter = settings.lateAfter; await db.query('UPDATE hrms_attendance_time_settings SET late_after = ?, absent_after = ? WHERE id = 1', [settings.lateAfter, settings.absentAfter]); } return { ...settings }; }
