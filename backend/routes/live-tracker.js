// routes/live-tracker.js

const express = require('express');
const router = express.Router();
const db = require('../config/db');

function formatDuration(seconds) {
    seconds = Number(seconds || 0);

    const hrs = Math.floor(seconds / 3600);
    const mins = Math.floor((seconds % 3600) / 60);

    if (hrs > 0 && mins > 0) {
        return `${hrs} hrs ${mins} mins`;
    }

    if (hrs > 0) {
        return `${hrs} hrs`;
    }

    return `${mins} mins`;
}

router.get('/:employeeName', async (req, res) => {
    const { employeeName } = req.params;
    const { date } = req.query;

    if (!employeeName) {
        return res.status(400).json({
            success: false,
            message: 'employeeName is required'
        });
    }

    try {
        const targetDate = date || new Date().toISOString().slice(0, 10);

        // 🟢 Fetch from live_tracking_history table for the specific date & employee
        const [rows] = await db.query(`
            SELECT
                lth.tracking_item_id,
                lth.task_list_id,
                lth.client_name,
                lth.employee_name,
                lth.task,
                lth.status,
                lth.started_at,
                lth.ended_at,
                lth.duration_secs,
                lth.manager_action,
                lth.manager_comment,
                tti.task_description,
                tti.comment
            FROM live_tracking_history lth
            LEFT JOIN time_tracking_task_items tti ON tti.id = lth.tracking_item_id
            WHERE lth.employee_name = ?
              AND DATE(lth.created_at) = ?
              AND lth.status != 'IDLE'
            ORDER BY lth.id ASC
        `, [employeeName, targetDate]);

        let totalWorkingSecs = 0;
        rows.forEach(row => {
            totalWorkingSecs += Number(row.duration_secs || 0);
        });

        const data = rows.map(row => {
            return {
                trackingItemId: row.tracking_item_id,
                taskListId: row.task_list_id,
                client_name: row.client_name || '',
                employee_name: row.employee_name,
                task: row.task || '',
                taskDescription: row.task_description || '',
                status: row.status || 'IDLE',
                duration: formatDuration(row.duration_secs),
                durationSecs: Number(row.duration_secs || 0),
                action: row.manager_action || 'ACTION',
                comment: row.manager_comment || row.comment || '',
                started_at: row.started_at,
                ended_at: row.ended_at
            };
        });

        return res.json({
            success: true,
            date: targetDate,
            totalWorkingSecs,
            totalWorkingTime: formatDuration(totalWorkingSecs),
            data
        });

    } catch (err) {
        console.error('GET /live-tracker ERROR:', err);
        return res.status(500).json({
            success: false,
            message: err.message
        });
    }
});

module.exports = router;