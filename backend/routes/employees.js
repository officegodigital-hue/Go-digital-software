...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    await db.query(`UPDATE hrms_employee_profiles SET full_name = ?, employee_code = ?, email = ?, department = ?
      WHERE employee_user_id = ?`, [fullName, staffId, email, role, empId]);
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    // Delete associated permissions first to prevent foreign key constraint restriction
      // Historical references must survive account deactivation.
