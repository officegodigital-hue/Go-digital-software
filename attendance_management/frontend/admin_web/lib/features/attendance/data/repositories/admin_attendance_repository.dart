...
typedef AdminAttendanceAccessTokenProvider = FutureOr<String?> Function();

class AdminAttendanceRepository {
  AdminAttendanceRepository({Dio? dio, this._accessTokenProvider})
    : _dio =
          dio ??
          Dio(
            BaseOptions(
  ...
  final AdminAttendanceAccessTokenProvider? _accessTokenProvider;

  Future<AdminAttendancePage> getAttendanceLog({
    int page = 1,
    int limit = 20,
    DateTime? fromDate,
    DateTime? toDate,
    ...
    _validatePagination(page: page, limit: limit);
    ...
    final dynamic responseData = await _executeRequest(() async {
    ...
    final Map<String, dynamic> attendancePayload = _extractAttendancePayload(
      responseData,
    );

    return AdminAttendancePage.fromJson(attendancePayload);
  }

  Future<AdminAttendancePage> fetchAttendanceLog({
    int page = 1,
    int limit = 20,
    DateTime? fromDate,
    DateTime? toDate,
  ...
  Future<List<AdminAttendanceRecord>> getAttendanceExportRecords({
    DateTime? fromDate,
    DateTime? toDate,
    String search = '',
    String status = 'all',
    ...
    return AdminAttendancePage.fromJson(attendancePayload).records;
  }

  Future<List<AdminAttendanceRecord>> fetchExportRecords({
    DateTime? fromDate,
    DateTime? toDate,
    String search = '',
    String status = 'all',
  ...
  Map<String, dynamic> _buildQueryParameters({
    ...
    final String normalizedSessionStatus = sessionStatus.trim().toLowerCase();
    ...
    return <String, dynamic>{
      ...
      if (normalizedSessionStatus.isNotEmpty &&
          normalizedSessionStatus != 'all')
        'session_status': normalizedSessionStatus,
      if (export) 'export': true,
    };
  }

  Future<Options> _authorizedOptions() async {
    final AdminAttendanceAccessTokenProvider tokenProvider =
        _accessTokenProvider ?? AdminAuthSession.getAccessToken;
  ...
  Future<dynamic> _executeRequest(
    ...
    try {
      ...
      return _validatedResponseData(response);
    } on AdminAttendanceRepositoryException {
      rethrow;
    } on DioException catch (error) {
      throw _mapDioException(error);
    } on FormatException catch (error) {
      throw AdminAttendanceRepositoryException(error.message);
    } catch (error) {
  ...
  dynamic _validatedResponseData(Response<dynamic> response) {
    ...
    if (data is Map) {
      ...
      if (success == false) {
        throw AdminAttendanceRepositoryException(
          _extractMessage(data) ?? 'The attendance request failed.',
  ...
  Map<String, dynamic> _extractAttendancePayload(dynamic data) {
    if (data is Map) {
      ...
      if (_looksLikeAttendancePayload(normalized)) {
        return normalized;
      }

      final dynamic nestedData = normalized['data'];

      if (nestedData != null) {
        final Map<String, dynamic> nestedPayload = _extractAttendancePayload(
          nestedData,
        );
      ...
      final dynamic result = normalized['result'];

      if (result != null) {
        final Map<String, dynamic> resultPayload = _extractAttendancePayload(
          result,
        );
  ...
  bool _looksLikeAttendancePayload(Map<String, dynamic> data) {
    return data.containsKey('records') ||
        data.containsKey('attendance_records') ||
        data.containsKey('attendanceRecords') ||
        data.containsKey('pagination') ||
        data.containsKey('summary');
  }

  AdminAttendanceRepositoryException _mapDioException(DioException error) {
    final int? statusCode = error.response?.statusCode;

    final dynamic responseData = error.response?.data;

    final String? serverMessage = _extractMessage(responseData);
    ...
    switch (error.type) {
      ...
      case DioExceptionType.badResponse:
        return AdminAttendanceRepositoryException(
          serverMessage ?? _messageForStatusCode(statusCode),
          statusCode: statusCode,
          responseData: responseData,
        );
  ...
  String? _extractMessage(dynamic data) {
    ...
    if (data is Map) {
      for (final String key in <String>['message', 'detail', 'error']) {
        ...
        if (value is Map) {
          final String? nestedMessage = _extractMessage(value);
  ...
  void _validatePagination({required int page, required int limit}) {
    if (page <= 0) {
      throw const AdminAttendanceRepositoryException(
...
class AdminAttendanceRepositoryException implements Exception {
