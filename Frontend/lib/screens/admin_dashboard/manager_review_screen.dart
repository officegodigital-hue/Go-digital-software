  ...
  Future<void> _fetchReviewData() async {
    ...
    try {
      ...
      if (r.statusCode == 200) {
        ...
        // Dispose any previously-built controllers before replacing the list.
          ...
          reviewData = rows.map((row) {
            ...
            return {
              ...
              "action": (row['manager_action'] as String?)?.isNotEmpty == true
                  ? row['manager_action']
                  : 'ACTION',
              "controller": TextEditingController(text: row['manager_comment'] as String? ?? ''),
            };
          }).toList();
          _loading = false;
        });
      } else {
        setState(() { _error = 'Failed to load reviews (${r.statusCode})'; _loading = false; });
      }
    } catch (e) {
