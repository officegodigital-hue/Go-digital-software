...
class VideographerTaskPlannerHistoryPage extends StatefulWidget {
  const VideographerTaskPlannerHistoryPage({super.key});
...
void initState() {
  ...
  Future<void> loadHistory() async {
   ...
   final List<VideographerHistoryItem> records =
    data.map<VideographerHistoryItem>((e) {
...
for (var item in records) {
  final date = DateTime.tryParse(item.sharedAt);
  ...
  if (!item.clientName.toLowerCase().contains(keyword) &&
  ...
  Future<void> updateTask(
    ...
    try {
      final r = await http.put(
        Uri.parse("$_baseUrl/videographer-planner/${item.id}"),
        headers: {
  ...
  Future<void> deleteTask(int id) async {
    ...
    try {
      final r = await http.delete(
        Uri.parse("$_baseUrl/videographer-planner/$id"),
        headers: {"Authorization": "Bearer $authToken"},
      );
...
void showEditDialog(VideographerHistoryItem item) {
          ...
          Row(
            children: [
              const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Videographer Task Planner History",
    style: AppTextStyles.heading),
                  SizedBox(height: 4),
                  Text(
                    "Review and manage videographer task planner history.",
              ...
              SizedBox(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: "Search client or scheduling...",
  ...
  child: const Row(
    children: [
      ...
      SizedBox(
        width: 180,
        child: Center(child: Text("Client Name", style: _headerStyle)),
      ),
      Expanded(
        child: Center(child: Text("Scheduling Details", style: _headerStyle)),
      ),
      SizedBox(
        width: 180,
  ...
  Container(
    ...
    alignment: Alignment.center,
    child: const Text(
      'No videographer planner history found.',
