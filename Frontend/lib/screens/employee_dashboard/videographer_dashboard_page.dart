// name=videographer_dashboard_page.dart
import 'dart:convert';
...
class VideographerDashboardPage extends StatefulWidget {
  ...
  const VideographerDashboardPage({
    super.key,
  ...
  @override
  State<VideographerDashboardPage> createState() =>
      _VideographerDashboardPageState();
}
...
class _VideographerDashboardPageState extends State<VideographerDashboardPage> {
  ...
  int holdedShoots = 0;
  List<dynamic> holdedShootsList = [];

  int inactiveTodayCount = 0;
  List<dynamic> inactiveTodayList = [];

  String globalFilterMode = 'Today'; // Hero section filter ('Today', 'This Week', 'This Month')
    ...
    try {
      ...
      if (r.statusCode == 200) {
        ...
        setState(() {
          ...
          holdedShootsList = data['onHoldList'] as List<dynamic>? ?? [];
 ...
 // 🟢 Clean and Unique Blue & White Gradient Popup matching your requirement
  void _showDetailsPopup(String title, List<dynamic> items, {bool isAssignedClient = false, bool isHoldCard = false, bool isInactiveCard = false}) {
    // Duplicate / repetitive client names-ai filter seithu unique items mattum edukkuvathu
    final uniqueItems = <String, dynamic>{};
    for (var item in items) {
      final name = (item['clientName'] ?? item['company_name'] ?? item['client_name'] ?? 'Client').toString();
                ...
                child: cleanItems.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final clientName = item['clientName'] ?? item['company_name'] ?? item['client_name'] ?? 'Client';
  ...
  // 🟢 Hero section date with global filter dropdown (Today, This Week, This Month)
  ...
  // 🟢 4 Cards setup (Assigned Clients, New Clients, Holded Shoots, Inactive Today)
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Wrap(
          ...
          runSpacing: 14,
          children: [
            // 1st Card: Assigned Clients (Unique active clients with allocated tasks)
            SizedBox(
              width: cardWidth,
            ...
            // 2nd Card: New Clients (Filtered based on top hero filter)
            SizedBox(
              width: cardWidth,
              child: GestureDetector(
            ...
            // 3rd Card: Holded Shoots
            SizedBox(
              width: cardWidth,
              child: GestureDetector(
                onTap: () => _showDetailsPopup('Holded Shoots', holdedShootsList, isHoldCard: true),
                child: MetricCard(
                  icon: Icons.pause_circle_outline,
                  title: 'Holded Shoots',
                  value: holdedShoots.toString().padLeft(2, '0'),
            ...
            SizedBox(
              ...
              child: GestureDetector(
                ...
                child: MetricCard(
                  ...
                  label: globalFilterMode, // 🟢 Label-il filter mode kaattum
                  color: AppColors.red,
                ),
              ),
            ),
  ...
  Widget _mobileTaskCard(VideographerTaskModel task) {
    return Container(
      width: double.infinity,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _headerCell(Icons.event_available_rounded, 'SHOOTING/EDITING DATE', 18),
  ...
  Widget _premiumTaskRow(VideographerTaskModel task, int index) {
...
class _PremiumTaskRow extends StatefulWidget {
  final VideographerTaskModel task;
  final int index;
...
class _PremiumTaskRowState extends State<_PremiumTaskRow> {
  ...
  VideographerTaskModel get task => widget.task;
  ...
  Widget _dueDateWidget(VideographerTaskModel task) {
