...
class _DesignerDashboardPageState extends State<DesignerDashboardPage> {
  ...
  // Only shown for tasks still open (mirrors the original hardcoded data,
  // where SUBMITTED/COMPLETED rows had no "days left" label).
  ...
  // Pushes the popup-menu selection to the backend, then updates the local
  // model on success so a refresh always reflects the real DB state.
  // Note: the DB status vocabulary is IDLE / IN PROGRESS / ON HOLD /
  // COMPLETED / REJECTED — there's no separate "SUBMITTED" status, so
  // "Submit for Review" reuses the complete endpoint and the badge will
  // show COMPLETED once persisted.
  Future<void> updateTask(DesignerTaskModel task, String endpointAction, String localAction, String localStatus) async {
