...
class _TrackingPageState extends State<TrackingPage> {
  ...
  _TrackingCounts counts = const _TrackingCounts(office: 0, home: 0, field: 0, activeNow: 0);
  bool loading = true;
  String? error;
...
Future<void> _openHomeApprovals() async {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      ...
      final countsJson = Map<String, dynamic>.from(data['counts'] as Map? ?? {});
      final items = (data['items'] as List? ?? [])
          .whereType<Map>()
          .map((item) => _TrackedEmployee.fromApi(Map<String, dynamic>.from(item)))
          .toList();
      if (!mounted) return;
      setState(() {
        employees = items;
          ...
          children: [
            ...
            Text(
              ...
              style: const TextStyle(
                  color: HrmsColors.navy, fontSize: 15, fontWeight: FontWeight.w700),
            ),
...
class _TrackingCounts {
  const _TrackingCounts(
      {required this.office, required this.home, required this.field, required this.activeNow});
...
class _TrackingKpis extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cards = [
      _TrackingKpi('Office', '${counts.office}', 'Tracked employees',
          Icons.apartment_outlined, HrmsColors.blue),
      _TrackingKpi('Home', '${counts.home}', 'Tracked employees',
          Icons.home_outlined, const Color(0xFF138A20)),
      _TrackingKpi('Field', '${counts.field}', 'Tracked employees',
          Icons.hiking_outlined, const Color(0xFFFF6500)),
      _TrackingKpi('Active Now', '${counts.activeNow}', 'Employees active',
...
class _LiveMap extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final located = employees.where((e) => e.lat != null && e.lng != null).toList();
        ...
        markers: located
            .map((employee) => Marker(
                  ...
                  infoWindow: InfoWindow(
                      title: employee.name,
                      snippet: employee.address ?? 'Location not yet reverse-geocoded'),
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: const Column(
          children: [
            ...
            SizedBox(height: 8),
            Text('No employees in this status right now',
              ...
              child:
                ...
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(employee.address ?? 'No location yet',
                        maxLines: 2,
                        overflow: TextOverflow.ellipsis,
                        style: const TextStyle(
                            color: Color(0xFF303747), fontSize: 12)),
                    Text(employee.updatedLabel,
...
class _TrackedEmployee {
  ...
  factory _TrackedEmployee.fromApi(Map<String, dynamic> json) {
    ...
    final lastUpdated = rawUpdated != null ? DateTime.tryParse(rawUpdated.toString()) : null;
...
class _RoutePoint {
  ...
  factory _RoutePoint.fromApi(Map<String, dynamic> json) {
    ...
    final time = recorded != null ? DateFormat('hh:mm a').format(recorded.toLocal()) : '--:--';
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(
                      child: Text('${widget.employee.name} — Today\'s Route',
                          style: const TextStyle(
                              color: HrmsColors.navy,
                              fontSize: 16,
                              fontWeight: FontWeight.w700)),
                ...
                Text('${widget.employee.id} · ${widget.employee.mode} mode',
                    style: const TextStyle(
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      setState(() {
        ...
        _pingController.text =
            '${data['field_ping_interval_minutes'] ?? 15}';
        _loading = false;
      });
    } catch (error) {
      if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Field tracking settings saved'),
        ),
      );
    } catch (error) {
      if (!mounted) return;
        ...
        content: SizedBox(
          ...
          child: _loading
              ...
              : _error != null
                  ...
                  : _items.isEmpty
                      ? const Center(
                          child: Text('No waiting reasons submitted yet.'),
                        )
                      : ListView.separated(
                          ...
                          itemBuilder: (_, index) {
                            ...
                            final fullName =
                                '${item['full_name'] ?? 'Unknown employee'}';
                            final employeeCode =
                                '${item['employee_code'] ?? ''}';
