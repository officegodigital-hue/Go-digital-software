...
class _EmployeeExtraHoursPageState extends State<EmployeeExtraHoursPage> {
  ...
  List<String> _getBaseUrls() {
    ...
    return [
      'http://localhost:3101/api/attendance',
      'http://localhost:3101/attendance',
                      ...
                      if (mounted) {
                        ...
                        if (ok) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Extra hours logged successfully!')),
                          );
                          _fetchData();
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Failed to submit extra hours.')),
                          );
                        }
                      }
                    },
  ...
  Widget build(BuildContext context) {
    final currentMonthFormatted = DateFormat('MMMM yyyy').format(DateTime.now());
            ...
            child: Column(
              ...
              children: [
                ...
                if (loading)
                  ...
                  SimpleTable(
                    minWidth: 790,
                    columns: const ['DATE', 'DAY', 'REGULAR OUT', 'ACTUAL OUT', 'EXTRA HRS'],
                    rows: rows.map((r) => [
                          Text(r['date_str']?.toString() ?? '',
                              style: const TextStyle(fontWeight: FontWeight.w700)),
                          Text(r['day_str']?.toString() ?? ''),
                          Text(r['regular_out']?.toString() ?? '—'),
                          Text(r['actual_out']?.toString() ?? '—'),
                          Text(r['hours_minutes']?.toString() ?? '',
                              style: const TextStyle(color: employeeBlue, fontWeight: FontWeight.w800)),
                        ]).toList(),
                  ),
                const Padding(
                  padding: EdgeInsets.fromLTRB(22, 14, 22, 16),
                  child: Row(children: [
                    Icon(Icons.info_outline_rounded, color: employeeMuted, size: 18),
                    SizedBox(width: 8),
                    Text('Extra hours are kept for attendance records only.',
  ...
  Widget build(BuildContext context) => EmployeeCard(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: const TextStyle(color: employeeMuted, fontSize: 12, fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            Text(value, style: const TextStyle(color: employeeNavy, fontSize: 28, fontWeight: FontWeight.w800)),
            const SizedBox(height: 3),
            Text(caption, style: const TextStyle(color: employeeMuted, fontSize: 13)),
          ]),
        ]),
      );
}
        ...
        child: Center(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              child: const Icon(Icons.schedule_outlined, color: employeeBlue, size: 31),
            ),
            const SizedBox(height: 12),
            const Text('No extra hours logged this month',
                style: TextStyle(color: employeeNavy, fontSize: 17, fontWeight: FontWeight.w800)),
            const SizedBox(height: 5),
            const Text('Use Log Hours to add a record.', style: TextStyle(color: employeeMuted)),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(monthLabel, style: const TextStyle(color: employeeMuted)),
                  ],
                ),
              ),
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                const Text('Total Extra Hours', style: TextStyle(color: employeeMuted)),
                const SizedBox(height: 4),
                Text(
                  metrics['this_month_hours'] ?? '0h',
                  style: const TextStyle(color: employeeOrange, fontSize: 28, fontWeight: FontWeight.w800),
                ),
              ]),
            ]),
            const Divider(height: 28, color: Color(0xFFFFCAA2)),
            Row(children: [
              Expanded(child: _ExtraSummary(Icons.schedule, 'This month', metrics['this_month_hours'] ?? '0h')),
              Expanded(child: _ExtraSummary(Icons.description_outlined, 'Sessions', rows.length.toString())),
            ]),
          ]),
        ),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (loading)
              ...
              const Padding(
                ...
                child: Center(child: Text('No extra hours recorded.', style: TextStyle(color: employeeMuted))),
              )
            else
...
class _ExtraSummary extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
        ...
        const SizedBox(height: 4),
        Text(value, style: const TextStyle(color: employeeOrange, fontSize: 18, fontWeight: FontWeight.w800)),
      ]);
}
...
class _MobileExtraRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: employeeLine))),
      child: Row(children: [
        const CircleAvatar(
          radius: 21,
          backgroundColor: Color(0xFFEAF2FF),
          child: Icon(Icons.calendar_today_outlined, color: employeeBlue, size: 21),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('${r['date_str']} 2026 · ${r['day_str']}',
                style: const TextStyle(color: employeeNavy, fontWeight: FontWeight.w800)),
            const SizedBox(height: 4),
            Text('${r['regular_out']} – ${r['actual_out']}',
                style: const TextStyle(color: employeeMuted, fontSize: 12)),
          ]),
        ),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text(r['hours_minutes']?.toString() ?? '',
              style: const TextStyle(color: employeeOrange, fontWeight: FontWeight.w800)),
        ]),
